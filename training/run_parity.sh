...
# Node ESM needs explicit .ts extensions; Metro does not, so we patch a copy.
sed "s#from './dsp'#from './dsp.ts'#g" "$ROOT/lib/mlClassifier.ts" > "$T/mlClassifier.ts"
...
const cfg={sampleRate:model.dsp.sampleRate,nfft:model.dsp.nfft,hop:model.dsp.hop,nMels:model.dsp.nMels,bandRatios:model.dsp.bandRatios,melFilterbank:model.dsp.melFilterbank};
